...
//	  			  }
//	  		  }
	  		  // limit velocity between -vmax and vmax
	  		  if(myRobot.v_r >= v_max){
	  			  myRobot.v_r = v_max;
	  		  }
	  		  else if(myRobot.v_r<= -v_max){
	  			  myRobot.v_r = -v_max;
	  		  }
	  		  if(myRobot.v_l >= v_max){
	  			  myRobot.v_l = v_max;
	  		  }
	  		  else if(myRobot.v_l <= -v_max){
	  			  myRobot.v_l = -v_max;
	  		  }

//		  	  sprintf(data,"!cmd:%s#v_r:%.2f#v_l:%.2f#none:100.00#\n",myRobot.cmd,myRobot.v_r,myRobot.v_l);
//		  	  HAL_UART_Transmit_DMA(&huart2, (uint8_t*)data,strlen(data));
