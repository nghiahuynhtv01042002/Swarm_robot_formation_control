...
//			update_Position_base_velocity(&myRobot, encoder_difference_left, encoder_difference_right,0.1);//delta time is 0.1
			...
			if((distance_to_goals < 0.03) ){ // it equal 0.03 before
...
//			sprintf(data,"!cmd:%s#enc_r:%d#enc_l:%d#\n",myRobot.cmd,encoder_value_right,encoder_value_left);
//			HAL_UART_Transmit_DMA(&huart2, (uint8_t*)data,strlen(data));
			//reset count
			count=0;
		}
...
//  pid_set_Kp_Ki_Kd(&hpid, 17, 1.5, 0.03,0.1);
//  pid_set_Kp_Ki_Kd(&hpid, 17, 1.5, 0.03,0.1); //raspi use this parameter
  pid_set_Kp_Ki_Kd(&hpid, 18, 1.5, 0.03 ,0.1);
